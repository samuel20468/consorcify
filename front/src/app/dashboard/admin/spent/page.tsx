        ...
        if (response.ok) {
          ...
          if (data.length > 0) {
            const consortiumExists = data.some(
              (c: { id: string | null }) => c.id === selectedConsortiumId
            );
            if (!consortiumExists) {
    ...
    const filterExpenditures = () => {
      ...
      if (selectedSupplierId) {
        filteredData = filteredData.filter(
          (expenditure) => expenditure.supplier?.id === selectedSupplierId
        );
      }
      ...
      <ContainerDashboard>
        ...
        <div className="flex items-center justify-between w-[98%]">
          ...
          <div className="flex w-1/3">
            <Link href="/addSpent" className="flex justify-end w-full mr-5">
        ...
        {suppliers.length > 0 && filteredExpenditures.length > 0 ? (
          ...
          <div className="p-8">
            <h1 className="text-2xl">Aún no hay gastos registrados</h1>
          </div>
        )}
      </ContainerDashboard>
    </div>
